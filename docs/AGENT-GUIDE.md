...
An **open-source**, git-native, static-output, agent-first publishing framework
for **developers, freelancers, and agencies**. Publish rich content and run a
blog **in context** with the website, app, or product it supports — without
leaving the IDE or inventing a separate CMS.

**Content is files** (Markdown + JSON), **the agent is the author**, **humans
approve via PR**, and the build emits **static HTML + a JSON/MD content API**
served from a CDN.
...
6. **AEO is compiled.** Schema/JSON-LD, `llms.txt`, sitemaps, markdown twins
   (with AEO headers), internal links are build outputs — don't hand-write them
   into bodies. Details: `docs/AEO.md`.
7. **Edge / CDN is human-gated.** Content negotiation Workers and host middleware
   are **not** part of the static engine. Agents must **ask the human and wait
   for approval** before deploying edge AEO (see `docs/AEO.md`). Never block a
   content PR on optional edge negotiation.
8. **Lean.** Reuse Astro + the host's primitives. Build only the unique 20%.
...
| AEO (static vs edge + human approval) | `docs/AEO.md` (synced to brands) |
| Brand upgrade after engine release | `docs/UPGRADE.md` (synced to brands) |
