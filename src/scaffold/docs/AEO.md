...
This is a reader signal, not a citation KPI. Google AIO / AI Mode eligibility is the GSC **Search generative AI** control (default **include**; worldwide as of **31 Aug 2026**; a `/blog` URL-prefix property inherits its parent). Measure impressions in the [GSC Generative AI performance report (Search)](https://support.google.com/webmasters/answer/16984139), not via `llms.txt` hits. A missing report can mean low impressions, exclude, or the help article’s remaining access gap — not lost AIO. Do not use Ahrefs AI-adjusted volume. Do not treat a GSC Generative AI logging window as lost AIO.
